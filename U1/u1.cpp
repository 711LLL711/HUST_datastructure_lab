...
*操作结果：构造一个空的线性表。
*返回值类型：status类型
*返回结果：线性表L不存在，构造一个空的线性表，返回OK，否则返回INFEASIBLE。
****************************************************************/
status InitList(SqList& L)
{
    if(L.elem){
...
*操作结果：销毁线性表L
*返回值类型：status类型
*返回结果：如果线性表L存在，销毁线性表L，释放数据元素的空间，返回OK，否则返回INFEASIBLE。
****************************************************************/
status DestroyList(SqList& L)
{
    if(L.elem==NULL){
...
*操作结果：将L重置为空表。
*返回值类型：status类型
*返回结果：如果线性表L存在，删除线性表L中的所有元素，返回OK，否则返回INFEASIBLE。
****************************************************************/
status ClearList(SqList& L)
{
    if(L.elem==NULL){
...
*返回值类型：status类型
*返回结果：如果线性表L存在，判断线性表L是否为空，空就返回TRUE，否则返回FALSE；如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status ListEmpty(SqList L)
{
    if(L.elem==NULL){
...
*操作结果：返回L中数据元素的个数。
*返回值类型：status类型
*返回结果：如果线性表L存在，返回线性表L的长度，否则返回INFEASIBLE。
****************************************************************/
status ListLength(SqList L)
{
    if(L.elem==NULL){
...
*返回值类型：status类型
*返回结果：如果线性表L存在，获取线性表L的第i个元素，保存在e中，返回OK；
          如果i不合法，返回ERROR；
...
*返回值类型：status类型
*返回结果：如果线性表L存在，查找元素e在线性表L中的位置序号并返回该序号；
          如果e不存在，返回ERROR；
...
*返回值类型：status类型
*返回结果：如果线性表L存在，获取线性表L中元素e的前驱，保存在pre中，返回OK；
          如果没有前驱，返回ERROR；
          如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status PriorElem(SqList L,ElemType e,ElemType &pre)
...
*返回值类型：status类型
*返回结果：如果线性表L存在，获取线性表L元素e的后继，保存在next中，返回OK；
          如果没有后继，返回ERROR；
          如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status NextElem(SqList L,ElemType e,ElemType &next)
...
*返回值类型：status类型
*返回结果：如果线性表L存在，将元素e插入到线性表L的第i个元素之前，返回OK；
          当插入位置不正确时，返回ERROR；
          如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status ListInsert(SqList &L,int i,ElemType e)
...
*返回值类型：status类型
*返回结果：如果线性表L存在，删除线性表L的第i个元素，并保存在e中，返回OK；
          当删除位置不正确时，返回ERROR；
          如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status ListDelete(SqList &L,int i,ElemType &e)
...
*操作结果：输出顺序表的值
*返回值类型：status类型
*返回结果：如果线性表L存在，依次显示线性表中的元素，每个元素间空一格，返回OK；
          如果线性表L不存在，返回INFEASIBLE。
****************************************************************/
status ListTraverse(SqList L)
{
...
*注释：将线性表保存到文件中
*返回值类型：status
*返回结果：如果线性表L存在，将线性表L的的元素写到FileName文件中，返回OK，否则返回INFEASIBLE。
****************************************************************/
status  SaveList(SqList L,char FileName[])
{
    FILE*p;
...
*操作结果：加载文件中的数据到线性表中
*返回值类型：status类型
*返回结果：如果线性表L不存在，将FileName文件中的数据读入到线性表L中，返回OK，否则返回INFEASIBLE。
****************************************************************/
status  LoadList(SqList &L,char FileName[])
{
    FILE* p;
...
*函数名称：Maxsubarray
*函数功能：找出有最大和的连续子数组
*算法思路：遍历线性表中的元素，运用贪心算法，如果当前累加和为0，更新结果，
...
*函数名称：subarraynum
*函数功能：找出和为k的连续子组的个数
*算法思路：遍历线性表中元素，依次作为起始点，计算从该起始点开始的连续子组的和，
          如果和为k，计数器加1，否则继续遍历
